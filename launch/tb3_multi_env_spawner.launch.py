...
def generate_launch_description():
    ...
    if verbose:
        ...
        print(
            f"[INFO] [{package_name}.launch.py] Available models: {env_available_models}"
        )
    ...
    gz_server_cmd = IncludeLaunchDescription(
        ...
        launch_arguments={"world": world, "verbose": str(gz_verbose)}.items(),
    )
    ...
    gz_client_cmd = IncludeLaunchDescription(
        ...
        launch_arguments={"verbose": str(gz_verbose)}.items(),
    )
    ...
    for i in range(num_envs):
        ...
        if random_pose:
            robot_init_pose = utils.get_random_pose(
                env_model_properties_path, env_center
            )
        else:
            robot_init_pose = [
                robot_pos_x + env_center[0],
